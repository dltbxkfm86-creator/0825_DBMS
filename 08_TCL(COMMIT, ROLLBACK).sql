/*
	...
	3. Isolation(고립성) : 동시에 실행되는 여러 트랜잭션이 상호간에 영향을 끼치지 않도록 하는 원칙
	
	4. Durablity(지속성) : 트랜잭션이 성공적으로 수행되었다면,
					     시스템에 문제가 바랭하더라도 영구적으로 반영이 되어야한다 	
...
SELECT * FROM EMP_COPY; -- 트랜잭션 아직 안 생김

DELETE FROM EMP_COPY WHERE EMP_ID = 900;
 ...
 * --> DDL을 써야하는데 이전 트랜잭션이 존재한다 -> COMMIT / ROLLBACK 수행 수 처리
 */
