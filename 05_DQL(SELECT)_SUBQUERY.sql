 ...
 * - 다중행 [단일열] 서브쿼리 : 서브쿼리 수행 결과가 여러 행일때
 * - [단일열] 다중열 서브쿼리 : 서브쿼리 수행 결과가 여러 열일때
...
SELECT
 ...
 GROUP
    BY
       DEPT_CODE; -- 830, 390, 266, 255, 289, 376, 750
...
-- 이승철 사원 또는 선승제 사원과 같은 부서인 사원들의 사원명, 휴대폰번호 조회
 ...
 WHERE
	   EMP_NAME IN (SELECT
					  ...
					  FROM
					       EMPLOYEE
					 WHERE
					       DEPT_CODE IN ('이승철', '선승제'));

SELECT * FROM JOB;
-- 인턴(수습사원) < 사원 < 주임 < 대리 < 과장 < 차장 < 부장
SELECT * FROM EMPLOYEE;
-- 대리직급임에도 불구하고 과장보다 급여를 많이 받는 대리가 존재
...
SELECT 
 ...
 WHERE J.JOB_CODE  = E.JOB_CODE
   AND JOB_NAME = '과장'; -- 220, 250, 232, 376, 750
...
SELECT
 ...
 WHERE
   ...
   AND 
       -- SALARY > 2200000 OR SALARY > 2500000
       SALARY > ANY (SELECT 
						    SALARY
					   FROM
					  	    EMPLOYEE E
...
-- 간단하고 시원하게 인라뷰 한 번 그냥 써보기
-- 사원들의 이름, 보너스 포함 연봉 조회하고 싶음
-- 단 보너스포함 연봉이 4000만원 이상은 사원만 조회
/*
SELECT
	   EMP_NAME AS "사원이름"
