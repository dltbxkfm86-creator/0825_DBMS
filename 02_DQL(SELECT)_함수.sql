...
SELECT
  ...
  FROM
  	   DUAL; -- 뒤에서부터 첫 번째 'B'가 앞에서부터 몇 번째 인지
  	   
SELECT
	   INSTR('AABAACAABBAA', 'B', 1, 3)
  FROM
       DUAL; -- 앞에서부터 세 번째 'B'가 앞에서부터 몇 번째 인지
...
SELECT 
	    EMP_NAME
	  , EMAIL
	  , SUSSTR(EMAIL, 1, INSTR(EMAIL, '@') - 1) "아이디"
  FROM
		EMPLOYEE
 WHERE
...
-- 각 직원별로 고용일로부터 오늘까지의 근무 일수 조회 + 이름조회
 ...
 -- ADD_MONTHS(DATE, NUMBER) : 특정날짜에 해당 숫자만큼의 개월 수를 더한 날짜(DATE로 변환)
 SELECT
 ...
 SELECT
 	  ...
 	  , NEXT_DAY(SYSDATE, '금')
 	  , NEXT_DAY(SYSDATE, 6) -- 1 : 일요일, 2 : 월요일, 3 : 화요일	
   FROM
   		DUAL;
 
 -- 언어 변경 ALTER SESSION SET NLS_LANGUAGE = AMERICAN
 ...
 */
 ...
 SELECT
 		EMP_NAME
 	  , DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남성',2, '여성', '성별 선택 안함') AS "성별"
   FROM
   		EMPLOYEE;
 ...
 /*
  * N개의 값을 읽어서 1개의 값을 반환 (하나의 그룹별로 함수 실행결과를 반환)
  */
