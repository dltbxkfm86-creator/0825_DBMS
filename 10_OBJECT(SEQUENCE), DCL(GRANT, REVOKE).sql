/*
 ...
 -- 회원번호, 사원번호, 게시글번호, 예약번호, 주문번호 등등 채번 할 때 주로 사용힘

	1. 시퀀스 객체 생성 구문
	
	[ 표현법 ]
	CREATE SEQUENCE 시퀀스명
	START WITH 시작 숫자 -> 시작값을 지정하는 옵션
	INCREMENT BY 증가값 -> 값을 증가시킬 때 몇 씩 증가할건지 지정하는 옵션
	MAXVALUE 최대값 -> 최대값 지정 옵션
	MINVALUE 최소값 -> 최소값 지정 옵션
	CYCLE / NOCYCLE -> 순환 여부 지정
	----------------- 생략가능
	CACHE 바이트크기 / NOCHACHE -> 캐시메모리 쓸건지 말건지
							 -> CACHE_SIZE 기본값은 20BYTE
	* CACHE : 미리 발생할 값을 생성해서 저장을 해둘건지 말건지 여부 지정
	* 		  값이 필요할 때 마다 매 번 새롭게 값을 생성해내는 것보다는 
	* 		  캐시공간에 미리 생성된 값들을 가져다 쓰는 것이 훨씬 속도적인 측면에서 이득임
	* 		  단, 접속이 끊기고나서 재접속 후 기존 생성된 값들이 다 날아가고 없음
	* 
*/
/*
 * * 접두사
...
/*
 * 2. 시퀀스를 사용하는 방법
 * 시퀀스명.CURRVAL : 현재 시퀀스의 값(마지막으로 성공으로 만들어진 NEXTVAL 값)
...
-- 수정 : ALTER  --> START WITH는 변경 불가능 너무 바꾸고싶으면 DROP하고 다시 CREATE
-- 삭제 : DROP

CREATE SEQUENCE SEQ_EID
 START WITH 224;

-- 사원을 추가해야할 때 마다 수행하는 INSERT
INSERT
  INTO
       EMPLOYEE
       (
