/*
 	...
 	오라클의 객체(구조) : 테이블(TABLE), 사용자(USER), 뷰(VIEW), 시퀀스(SEQUENCE)
					...
					, 프로시저(PROCEDURE), 함수(FUNCTION), 동의어(SYNONYM) ...ALTER 
	...
	테이블이란? : 행(ROW), 열(COLUME)로 구성되는 가장 기본적인 데이터베이스 객체
	...
	- CHAR(바이트크기) : 최대 2000BYTE까지 지정가능
					   고정길이(아무리 작은 값이 들어와도 공백으로 채워서 크기 유지) 정적
 ...
 * 컬럼에 주석 달기 == 설명 다는 법
 * 
 * COMMENT ON COLUME 테이블명.컬럼명 IS '설명'
 */
...
-- 현재 이 계정이 가지고 있는 테이블의 모든 컬럼의 정보를 조회할 수 있음
 ...
 * - 제약조건을 부여하면 데이터를 INSERT / UPDATE할 때 마다 제약조건에 위배되지 않는지 검사
 * 
...
VALUES
	 ...
	 , NULL
	 , NULL
	 ); -- NOT NULL 제약조건이 달린 컬럼에는 반드시 NULL이 아닌 값이 존재해야 함
...
-- CHECK를 부여하더라도 NULL값은 INSERT할 수 있음 -> NOT NULL로 막아야 함
-------------------------------------------------------------------
/*
 * 테이블 만들기 실습
 ...
 */
--특정 컬럼에 기본값을 설정하는 방법 ===> 제약 조건 아님
CREATE TABLE TB_USER_DEFAULT(
	USER_NO NUMBER NOT NULL ,
	USER_ID VARCHAR2(20) NOT NULL UNIQUE,
	USER_PWD VARCHAR2(20) NOT NULL,
	USER_NAME NVARCHAR2(30),
	USER_NICKNAME NVARCHAR2(20) UNIQUE,
 ...
 * => 중복이 발생해선 안되고 값이 꼭 있어야만 하는 식별용 컬럼에 PRIMARYY KEY를 부여
 * 
 * 한 테이블 당 한 번만 사용 가능
 */
...
-- 기본키 컬럼은 중복값을 INSERT할 수 없음

INSERT INTO TB_USER_PK
VALUES (NULL, 'user02', '3535', NULL, NULL);
-- 기본 키 컬럼은 NULL값을 INSERT할 수 없음

CREATE TABLE TB_PRIMARTKEY(
...
-- 조인을 하기 위해서 꼭 외래키 제역조건을 달아야하는 것은 아님!
 ...
 * 
 * 제약조건 달려있는 걸 삭제
 * PRIMARY KEY, FOREIGN KEY, UNIQUE, CHECK : DROP CONSTAINT 제약조건명
 * 
...
-- 3_1) 컬럼명 바꾸기 : ALTER TABLE 테이블명 RENAME COLUME 원래컬럼명 TO 바꿀컬럼명

ALTER TABLE JON_COPY RENAME COLUMN LOCATION TO LNAME;
